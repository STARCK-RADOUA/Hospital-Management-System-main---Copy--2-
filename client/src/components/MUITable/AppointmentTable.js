...
function createData(patientName, doctorName, appointmentDate, appointmentTime,appointmentType, actionsID) {
    return { patientName, doctorName, appointmentDate, appointmentTime, appointmentType,actionsID };
}
...
export default function AppointmentTable({ bookedAppointments, deleteBookedSlots, doctorList, patientList, availableSlots, getAvailableSlots, getBookedSlots }) {
    ...
    const [appointmentType, setAppointmentType] = React.useState('hopital');
    ...
    const setFormProperties = async (appID) => {
        ...
        setAppointmentType(app.appointmentType);
    ...
    let rows = bookedAppointments.map((apt) => {
        ...
        const patientName = apt.patientId ? `${apt.patientId.userId.firstName} ${apt.patientId.userId.lastName}` : '';
    const doctorName = apt.doctorId ? `${apt.doctorId.userId.firstName} ${apt.doctorId.userId.lastName}` : '';
    return createData(
        patientName,
        doctorName,
